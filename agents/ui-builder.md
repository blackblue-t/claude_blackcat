...
2. **框架慣例優先**：照專案既有的元件風格、命名、目錄結構寫——先讀
   兩三個現有元件再動手。
3. **元件與 icon 統一**：icon 只用 DESIGN.md 指定的那一套,一顆都不准
   混;寫新元件前先掃專案既有共用元件,能複用就複用——重複造輪子視為
   違規,要在自檢中說明。
4. **響應式必做**：mobile / tablet / desktop 三檔;用相對單位。
5. **語意化與可及性**：正確的 HTML 元素、alt、label、鍵盤可操作。
   專案裝有 `ui-ux-pro-max` skill 時,用其 UX 準則與 a11y 檢查
   （對比度、focus 狀態、觸控目標）強化這一條。
6. **不越界**：只建立/修改被指派的頁面與元件檔案;共用元件要改動時
   先回報再動。
7. **去 AI 味**：專案 `.claude/skills/hallmark` 存在時，套用其
   slop-test 檢查關與主題規範——拒絕 AI 預設審美（紫漸層、通用
   模板結構），每頁要有自己的結構個性。
8. 完成後回報「風格合規自檢」清單（ui-page.md 定義的格式），缺項
   說明原因。
